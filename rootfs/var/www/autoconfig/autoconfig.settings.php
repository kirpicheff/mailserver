...
$mdomain = isset($_SERVER['HTTP_HOST'])?$_SERVER['HTTP_HOST']:null;
if (is_null($domain) && $_SERVER['argc']>1)
        $domain=$_SERVER['argv'][1];
if (is_null($domain))
        die("No domain provided");
$mdomain = strtolower( str_replace("autoconfig.","",$domain) );


$cfg = $config->add($mdomain);

// Name and short name for the offered service, as used by Mozilla Thunderbird
$cfg->name = 'mail services';
$cfg->nameShort = 'MailServer';
